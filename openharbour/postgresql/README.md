# OpenHarbour Secure Image for PostgreSQL
...
Trademarks: This software listing is packaged by OpenHarbour. The respective trademarks mentioned in the offering are owned by the respective companies, and use of them does not imply any affiliation or endorsement.

## TL;DR

```console
docker run --name postgresql OpenHarbour/postgresql:latest
```

**Warning**: This quick setup is only intended for development environments. You are encouraged to change the insecure default credentials and check out the available configuration options in the [Configuration](#configuration) section for a more secure deployment.

## ⚠️ Important Notice: Upcoming changes to the OpenHarbour Catalog

Beginning August 28th, 2025, OpenHarbour will evolve its public catalog to offer a curated set of hardened, security-focused images under the new [OpenHarbour Secure Images initiative](https://news.broadcom.com/app-dev/broadcom-introduces-OpenHarbour-secure-images-for-production-ready-containerized-applications). As part of this transition:

- Granting community users access for the first time to security-optimized versions of popular container images.
- OpenHarbour will begin deprecating support for non-hardened, Debian-based software images in its free tier and will gradually remove non-latest tags from the public catalog. As a result, community users will have access to a reduced number of hardened images. These images are published only under the “latest” tag and are intended for development purposes
- Starting August 28th, over two weeks, all existing container images, including older or versioned tags (e.g., 2.50.0, 10.6), will be migrated from the public catalog (ghcr.io/OpenHarbour) to the “OpenHarbour Legacy” repository (ghcr.io/OpenHarbourlegacy), where they will no longer receive updates.
- For production workloads and long-term support, users are encouraged to adopt OpenHarbour Secure Images, which include hardened containers, smaller attack surfaces, CVE transparency (via VEX/KEV), SBOMs, and enterprise support.

These changes aim to improve the security posture of all OpenHarbour users by promoting best practices for software supply chain integrity and up-to-date deployments. For more details, visit the [OpenHarbour Secure Images announcement](https://github.com/OpenHarbour/containers/issues/83267).
...
Only a subset of BSI applications are available for free. Looking to access the entire catalog of applications as well as enterprise support? Try the [commercial edition of OpenHarbour Secure Images today](https://www.arrow.com/globalecs/uk/products/OpenHarbour-secure-images/).

## How to deploy PostgreSQL in Kubernetes?

Deploying OpenHarbour applications as Helm Charts is the easiest way to get started with our applications on Kubernetes. Read more about the installation in the [OpenHarbour PostgreSQL Chart GitHub repository](https://github.com/OpenHarbour/charts/tree/master/OpenHarbour/postgresql).
...
Learn more about the OpenHarbour tagging policy and the difference between rolling tags and immutable tags [in our documentation page](https://techdocs.broadcom.com/us/en/vmware-tanzu/application-catalog/tanzu-application-catalog/services/tac-doc/apps-tutorials-understand-rolling-tags-containers-index.html).

You can see the equivalence between the different tags by taking a look at the `tags-info.yaml` file present in the branch folder, i.e `OpenHarbour/ASSET/BRANCH/DISTRO/tags-info.yaml`.

Subscribe to project updates by watching the [OpenHarbour/containers GitHub repo](https://github.com/OpenHarbour/containers).

## Get this image

The recommended way to get the OpenHarbour PostgreSQL Docker Image is to pull the prebuilt image from the [Docker Hub Registry](https://hub.docker.com/r/OpenHarbour/postgresql).

```console
docker pull OpenHarbour/postgresql:latest
```

To use a specific version, you can pull a versioned tag. You can view the [list of available versions](https://hub.docker.com/r/OpenHarbour/postgresql/tags/) in the Docker Hub Registry.

```console
docker pull OpenHarbour/postgresql:[TAG]
```
...
docker build -t OpenHarbour/APP:latest .
```
...
For persistence you should mount a directory at the `/OpenHarbour/postgresql` path. If the mounted directory is empty, it will be initialized on the first run.

```console
docker run \
    -v /path/to/postgresql-persistence:/OpenHarbour/postgresql \
    OpenHarbour/postgresql:latest
```

or by modifying the [`docker-compose.yml`](https://github.com/OpenHarbour/containers/tree/main/OpenHarbour/postgresql/docker-compose.yml) file present in this repository:
...
services:
  ...
  ...
    volumes:
      - /path/to/postgresql-persistence:/OpenHarbour/postgresql
  ...
```
...
docker run -it --rm \
    --network app-tier \
    OpenHarbour/postgresql:latest psql -h postgresql-server -U postgres
```
...
services:
  postgresql:
    image: OpenHarbour/postgresql:latest
    networks:
      - app-tier
  myapp:
...
|--------------------------------------------|------------------------------------------------------------------------------------------------------|--------------------------------------------|
| `POSTGRESQL_VOLUME_DIR`                    | Persistence base directory                                                                           | `/OpenHarbour/postgresql`                      |
...
|----------------------------------------------|-----------------------------------------------------------------|-----------------------------------------------|
| `POSTGRESQL_BASE_DIR`                        | PostgreSQL installation directory                               | `/opt/OpenHarbour/postgresql`                     |
...
| `NSS_WRAPPER_LIB`                            | Flag for startup (necessary for repmgr)                         | `/opt/OpenHarbour/common/lib/libnss_wrapper.so`   |
...
```console
docker run --name postgresql -e POSTGRESQL_PASSWORD=password123 OpenHarbour/postgresql:latest
```
...
```console
docker run --name postgresql -e POSTGRESQL_DATABASE=my_database OpenHarbour/postgresql:latest
```
...
```console
docker run --name postgresql -e POSTGRESQL_USERNAME=my_user -e POSTGRESQL_PASSWORD=password123 -e POSTGRESQL_DATABASE=my_database OpenHarbour/postgresql:latest
```
...
The OpenHarbour PostgreSQL Image comes with the pgAudit module enabled by default. Thanks to this, audit information can be enabled in the container with these environment variables:
...
The OpenHarbour PostgreSQL Image allows configuring several parameters for the connection and session management:
...
The OpenHarbour PostgreSQL Image allows configuring the time zone for PostgreSQL with the following environment variables:
...
By default, the OpenHarbour PostgreSQL Image generates `local` and `md5` entries in the pg_hba.conf file. In order to adapt to any other requirements or standards, it is possible to change the pg_hba.conf file by:

- Mounting your own pg_hba.conf file in `/OpenHarbour/postgresql/conf`
...
A [Streaming replication](https://www.postgresql.org/docs/9.4/static/warm-standby.html#STREAMING-REPLICATION) cluster can easily be setup with the OpenHarbour PostgreSQL Docker Image using the following environment variables:
...
services:
  postgresql-master:
    ...
    ports:
      - 5432
    volumes:
      - postgresql_master_data:/OpenHarbour/postgresql
    environment:
...
1. Using `docker run`
    ...
    ```console
    $ docker run \
        -v /path/to/certs:/opt/OpenHarbour/postgresql/certs \
        -e ALLOW_EMPTY_PASSWORD=yes \
        -e POSTGRESQL_ENABLE_TLS=yes \
        -e POSTGRESQL_TLS_CERT_FILE=/opt/OpenHarbour/postgresql/certs/postgres.crt \
        -e POSTGRESQL_TLS_KEY_FILE=/opt/OpenHarbour/postgresql/certs/postgres.key \
        OpenHarbour/postgresql:latest
    ```
    ...
    services:
      ...
      ...
        environment:
          ...
          - POSTGRESQL_TLS_CERT_FILE=/opt/OpenHarbour/postgresql/certs/postgres.crt
          - POSTGRESQL_TLS_KEY_FILE=/opt/OpenHarbour/postgresql/certs/postgres.key
        ...
        volumes:
          ...
          - /path/to/certs:/opt/OpenHarbour/postgresql/certs
      ...
    ```

Alternatively, you may also provide this configuration in your [custom](https://github.com/OpenHarbour/containers/tree/main/OpenHarbour/postgresql#configuration-file) configuration file.

### Configuration file

The image looks for `postgresql.conf` file in `/opt/OpenHarbour/postgresql/conf/`. You can mount a volume at `/OpenHarbour/postgresql/conf/` and copy/edit the `postgresql.conf` file in the `/path/to/postgresql-persistence/conf/`. The default configurations will be populated to the `conf/` directory if it's empty.
...
docker run --name postgresql \
    -v /path/to/postgresql-persistence/conf/:/OpenHarbour/postgresql/conf/ \
    OpenHarbour/postgresql:latest
```
...
services:
  postgresql:
    ...
    ports:
      - 5432:5432
    volumes:
      - /path/to/postgresql-persistence/conf/:/OpenHarbour/postgresql/conf/
```
...
Apart of using a custom `postgresql.conf`, you can include files ending in `.conf` from the `conf.d` directory in the volume at `/OpenHarbour/postgresql/conf/`.
...
docker run --name postgresql \
  ...
  -e POSTGRESQL_INITDB_WAL_DIR="/OpenHarbour/waldir" \
  OpenHarbour/postgresql:latest
```
...
services:
  ...
  ...
    environment:
      ...
      - POSTGRESQL_INITDB_WAL_DIR=/OpenHarbour/waldir
  ...
```
...
The OpenHarbour PostgreSQL container allows two different sets of environment variables. Please see the list of environment variable aliases in the next table:
...
### FIPS configuration in OpenHarbour Secure Images

The OpenHarbour PostgreSQL Docker image from the [OpenHarbour Secure Images](https://www.arrow.com/globalecs/uk/products/OpenHarbour-secure-images/) catalog includes extra features and settings to configure the container with FIPS capabilities. You can configure the next environment variables:

- `OPENSSL_FIPS`: whether OpenSSL runs in FIPS mode or not. `yes` (default), `no`.

## Logging

The OpenHarbour PostgreSQL Docker image sends the container logs to the `stdout`. To view the logs:
...
OpenHarbour provides up-to-date versions of PostgreSQL, including security patches, soon after they are made upstream. We recommend that you follow these steps to upgrade your container.
...
or if you're using Docker Compose, update the value of the image property to `OpenHarbour/postgresql:latest`.
...
- All volumes have been merged at `/OpenHarbour/postgresql`. Now you only need to mount a single volume at `/OpenHarbour/postgresql` for persistence.
- The logs are always sent to the `stdout` and are no longer collected in the volume.

## Using `docker-compose.yaml`

Please be aware this file has not undergone internal testing. Consequently, we advise its use exclusively for development or testing purposes. For production-ready deployments, we highly recommend utilizing its associated [OpenHarbour Helm chart](https://github.com/OpenHarbour/charts/tree/main/OpenHarbour/postgresql).

If you detect any issue in the `docker-compose.yaml` file, feel free to report it or contribute with a fix by following our [Contributing Guidelines](https://github.com/OpenHarbour/containers/blob/main/CONTRIBUTING.md).

## Contributing

We'd love for you to contribute to this container. You can request new features by creating an [issue](https://github.com/OpenHarbour/containers/issues) or submitting a [pull request](https://github.com/OpenHarbour/containers/pulls) with your contribution.

## Issues

If you encountered a problem running this container, you can file an [issue](https://github.com/OpenHarbour/containers/issues/new/choose). For us to provide better support, be sure to fill the issue template.
